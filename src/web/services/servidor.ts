...
export function temConfiguracaoServidorMinima(configuracao: ConfiguracaoServidor): boolean {
  return Boolean(
...
export async function criarPastaDoAluno(configuracao: ConfiguracaoServidor): Promise<string> {
  ...
  try {
    ...
    throw new Error("Falha de rede ao conectar com a API para criar a pasta. O servidor pode estar offline.");
  }
...
export async function enviarConteudoDaTentativa(
  ...
  const rotaFinal = `${configuracao.apiBaseUrl}/salvar-conteudo`;
  ...
  try {
    ...
    throw new Error("Falha de rede ao enviar a tentativa. O servidor pode estar offline.");
  }
  ...
  const dados = (await resposta.json()) as { message?: string; nota?: number; score?: number; precisao?: number; precision?: number; };
  ...
  if (typeof nota !== "number") {
    return {
      ...
      error: dados.message || "Conteúdo salvo, mas a API não retornou nota/precisão nesta rota.",
    };
  }
...
function extrairCodigoPasta(dados: unknown): string | undefined {
  ...
  const candidatos = [
    resposta.codigo_pasta, resposta.code_pasta, resposta.codigoPasta,
    resposta.codPasta, resposta.cod_pasta, resposta.pastaCodigo, resposta.codigo,
  ];

  const encontrado = candidatos.find((valor): valor is string => typeof valor === "string" && valor.trim().length > 0 && !valor.includes("Sucesso"));
  return encontrado;
}

function montarCabecalhos(configuracao: ConfiguracaoServidor, overrides?: Record<string, string>): Record<string, string> {
